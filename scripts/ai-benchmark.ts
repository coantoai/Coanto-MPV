...
type Result = { provider: Provider; model: string; caseId: string; competitors: CompetitorResult[]; sources: string[]; raw: string; latencyMs: number; error?: string };

const providerKeys: Record<Provider, string> = { openai: 'OPENAI_API_KEY', gemini: 'GEMINI_API_KEY', openrouter: 'OPENROUTER_API_KEY', anthropic: 'ANTHROPIC_API_KEY' };
...
async function callProvider(provider: Provider, prompt: string) {
  ...
  if (provider === 'openai') {
    ...
    const data = await response.json(); return { text: extractText(data), sources: extractSources(data) };
  }
  if (provider === 'gemini') {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(models.gemini)}:generateContent?key=${encodeURIComponent(key)}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }] }], tools: [{ google_search: {} }], generationConfig: { responseMimeType: 'application/json' } }) });
...
function parseCompetitors(text: string): CompetitorResult[] { const start = text.indexOf('{'); const end = text.lastIndexOf('}'); if (start < 0 || end <= start) return []; try { const parsed = JSON.parse(text.slice(start, end + 1)); const values = parsed.competitors ?? parsed.competitor_domains ?? []; if (!Array.isArray(values)) return []; return values.map((value: any) => typeof value === 'string' ? { domain: value } : value).filter((value: any) => value && typeof value === 'object').slice(0, 5); } catch { return []; } }
function competitorValue(item: CompetitorResult) { return item.domain || item.url || item.name || ''; }
function score(found: CompetitorResult[], expected: string[]) { const f = new Set(found.map(competitorValue).map(normalizeCompetitor).filter(Boolean)); const e = new Set(expected.map(normalizeCompetitor).filter(Boolean)); const hits = [...f].filter((item) => [...e].some((target) => item === target || item.endsWith(`.${target}`) || target.endsWith(`.${item}`))).length; const grounded = found.filter((item) => Array.isArray(item.sourceUrls) && item.sourceUrls.length > 0).length; const evidenced = found.filter((item) => typeof item.evidence === 'string' && item.evidence.trim().length > 0).length; return { hits, precisionAt5: found.length ? hits / Math.min(found.length, 5) : 0, recallAt5: hits / Math.max(e.size, 1), evidenceCoverage: found.length ? (grounded + evidenced) / (2 * found.length) : 0 }; }
...
for (const provider of providers) {
  ...
  let providerStopped: string | undefined;
  for (const test of benchmarkCases) {
    ...
    try {
      ...
      if (/\b(?:401|403|404|429|quota|rate.?limit|insufficient_quota)\b/i.test(message)) { providerStopped = message.slice(0, 240); break; }
    }
  }
  if (providerStopped) console.log(`STOP ${provider}: ${providerStopped}; remaining cases skipped to avoid wasting quota.`);
}

const summary = providers.map((provider) => { const rows = results.filter((r) => r.provider === provider && !r.error); const scored = rows.map((row) => score(row.competitors, benchmarkCases.find((c) => c.id === row.caseId)?.expectedCompetitors ?? [])); return { provider, model: models[provider], status: process.env[providerKeys[provider]] ? (rows.length ? 'tested' : 'failed'), cases: rows.length, failedCases: results.filter((r) => r.provider === provider && Boolean(r.error)).length, meanPrecisionAt5: scored.length ? scored.reduce((a, b) => a + b.precisionAt5, 0) / scored.length : null, meanRecallAt5: scored.length ? scored.reduce((a, b) => a + b.recallAt5, 0) / scored.length : null, meanEvidenceCoverage: scored.length ? scored.reduce((a, b) => a + b.evidenceCoverage, 0) / scored.length : null, meanLatencyMs: rows.length ? rows.reduce((a, b) => a + b.latencyMs, 0) / rows.length : null }; });
const successfulCases = results.filter((result) => !result.error && result.raw.trim().length > 0).length;
const report = { generatedAt: new Date().toISOString(), benchmarkVersion: '2026-09-v7', cases: benchmarkCases.length, status: successfulCases > 0 ? 'tested' : configured.length ? 'failed-all-configured-providers' : 'blocked-missing-provider-secrets', configuredProviders: configured, successfulCases, summary, results };
