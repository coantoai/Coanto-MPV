...
// /live is the real product test surface; /next remains preserved separately.
...
const liveResultsCss = readFileSync("src/components/decision-experience/live-product-results.css", "utf8");
...
assert.ok(liveRoute.includes("live-product-results.css?url"));
...
assert.ok(liveProduct.includes("راقب السوق. افهم ما تغيّر. قرّر."));
assert.ok(liveProduct.includes("المشهد التنافسي"));
assert.ok(liveProduct.includes("الإشارات الأهم"));
assert.ok(liveProduct.includes("الفرص والتهديدات والخطوات المقترحة"));
assert.ok(liveProduct.includes("المصادر التي جمعها COANTO"));
assert.ok(liveProduct.includes("مصادر مرتبطة بهذا القرار تحديدًا"));
assert.ok(liveProduct.includes("ما يدعمه، ما يعارضه، وما لا نعرفه"));
assert.ok(liveProduct.includes('root["beforeAfter"]'));
assert.ok(liveProduct.includes('root["opportunities"]'));
assert.ok(liveProduct.includes('root["threats"]'));
assert.ok(liveProduct.includes('root["actions"] ?? root["action_plan"]'));
assert.ok(liveProduct.includes("answerLiveDecisionQuestion"));
assert.ok(liveProduct.includes("localStorage.setItem"));
assert.ok(liveProduct.includes("التحليلات السابقة"));
assert.ok(!liveProduct.includes("Evidence Gate"));
assert.ok(!liveProduct.includes("R1–R60"));
assert.ok(!liveProduct.includes("LIVE E2E"));
assert.ok(!liveProduct.includes("تفسير AI"));
...
assert.ok(liveResultsCss.includes(".lp-overview"));
assert.ok(liveResultsCss.includes(".lp-change-grid"));
assert.ok(liveResultsCss.includes(".lp-insight-grid"));
assert.ok(liveResultsCss.includes(".lp-source-grid-rich"));
...
console.log("PASS: preserved preview, product-only live analysis, full-result visuals, source transparency, server-gated decisions, safe links, tenant ownership and auth-return contract.");
