...
export const refreshBusinessIntelligence=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).handler(async({context})=>{
  const now=new Date(); const since=new Date(now.getTime()-30*24*3600000).toISOString();
  const [{data:analyses,error:aError},{data:events,error:eError}]=await Promise.all([
  ...
  if(aError)throw new Error(aError.message); if(eError)throw new Error(eError.message);
  const analysisRows=analyses??[], eventRows=events??[];
  ...
  const {error:metricError}=await context.supabase.from("business_metrics").insert(metrics.map(([metric_key,metric_value,unit])=>({user_id:context.userId,metric_key,metric_value,unit,period_start:since,period_end:now.toISOString(),source_analysis_id:sourceAnalysisId,metadata:{derivedFrom:"analyses+monitoring_events",windowDays:30}})));
  if(metricError)throw new Error(metricError.message);
  if(changes.length){
    const first=changes[0] as any;
    const {error}=await context.supabase.from("business_insights").insert({user_id:context.userId,title:"السوق التنافسي شهد تغيّرات حديثة",summary:`تم رصد ${changes.length} تغيّرًا تنافسيًا خلال آخر 30 يومًا. هذه إشارة للمراجعة وليست إثباتًا لأثر مالي.`,category:"competition",impact:highRisk.length?"high":"medium",confidence:Math.min(0.95,0.55+Math.min(changes.length,8)*0.05),evidence:[{type:"monitoring-events",count:changes.length,latest:first.title,evidence:first.evidence??{}}],recommendation:"راجع التغيّرات المؤكدة أولًا، ثم اربطها بأدلة المنتج والسعر قبل اتخاذ قرار.",source_analysis_id:sourceAnalysisId});
    if(error)throw new Error(error.message);
  } else if(analysisRows.length){
    const {error}=await context.supabase.from("business_insights").insert({user_id:context.userId,title:"بيانات COANTO جاهزة للتحليل التجاري",summary:`يوجد ${analysisRows.length} تحليلًا محفوظًا خلال آخر 30 يومًا، ولا توجد تغيّرات مراقبة مؤكدة في الفترة نفسها.`,category:"operations",impact:"low",confidence:0.75,evidence:[{type:"analysis-count",count:analysisRows.length}],recommendation:"شغّل المراقبة على المنافسين الأساسيين لبناء خط زمني للتغيّرات.",source_analysis_id:sourceAnalysisId});
    if(error)throw new Error(error.message);
  }
  return {ok:true,analyses:analysisRows.length,events:eventRows.length,changes:changes.length,errors:errors.length};
});
