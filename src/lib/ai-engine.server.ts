...
const jsonInstruction = `Return one JSON object only; no markdown before or after it. Do not invent facts. Every material claim must be traceable to supplied evidence or a web source. Distinguish observed facts, estimates, inferences, recommendations, counter-evidence, and unknowns. Prefer explicit uncertainty over filling gaps. The response must contain a non-empty competitors array; each competitor must include name, canonical official homepage URL, why, evidence, relevance (0-100), impact (0-100), and threat (critical|high|medium|low). Also return signals, priority_matrix, threats, opportunities, scenarios, action_plan, trust, unknowns, summary, next_action, threat_level, and opportunity_level. priority_matrix.zone must be exactly one of do-now, test, monitor, ignore. Every priority_matrix item must include why, sourceUrls, evidenceFor, counterEvidence, trigger, and nextAction. sourceUrls must support that specific decision row; never attach unrelated URLs merely to decorate a recommendation. evidenceFor and counterEvidence must contain only evidence actually found; use an empty counterEvidence array when no opposing evidence is verified and surface the gap in unknowns. trigger must be an observable condition that would reopen or change the decision, never an invented date, probability, threshold, financial effect, margin, or price guardrail. nextAction must be bounded and reversible when internal constraints are missing; do not prescribe exact pricing, spend, margin, inventory, or other irreversible actions unless supplied business context supports them. If a row lacks decision-linked evidence, return sourceUrls as an empty array so the product can classify it as insufficient evidence instead of promoting it. action_plan.timing must be exactly one of today, week, later. Each signal must include title, description, competitor when applicable, impact, confidence, and sourceUrls when available. Search only when the supplied evidence is insufficient; avoid repeated searches for the same fact. All user-facing prose must be clear, concise Arabic suitable for a small-business owner; avoid unexplained technical jargon. Never replace missing evidence with a guess and never invent confidence percentages.`;
...
const matrixSchema = { type: 'OBJECT', properties: { title: { type: 'STRING' }, zone: { type: 'STRING' }, why: { type: 'STRING' }, impact: { type: 'STRING' }, urgency: { type: 'STRING' }, sourceUrls: { type: 'ARRAY', items: { type: 'STRING' } }, evidenceFor: { type: 'ARRAY', items: { type: 'STRING' } }, counterEvidence: { type: 'ARRAY', items: { type: 'STRING' } }, trigger: { type: 'STRING' }, nextAction: { type: 'STRING' } }, required: ['title','zone','why','sourceUrls','evidenceFor','counterEvidence','trigger','nextAction'] };
