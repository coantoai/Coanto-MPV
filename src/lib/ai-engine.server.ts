...
async function runOpenAi(prompt: string): Promise<AiRun> {
  ...
  const response = await fetch('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({ model, input: `${jsonInstruction}\n\n${prompt}`, tools: [{ type: 'web_search_preview' }], include: ['web_search_call.action.sources'] }),
  });
  if (!response.ok) throw new Error(`OpenAI API ${response.status}: ${await response.text()}`);
  const data = await response.json();
  const text = textFromOpenAi(data);
  if (!text) throw new Error('OpenAI returned no text output.');
  const sources = (data?.output ?? []).filter((item: any) => item?.type === 'web_search_call').flatMap((item: any) => item?.action?.sources ?? []).map((source: any) => source?.url).filter((url: unknown): url is string => typeof url === 'string') as string[];
...
async function runGemini(prompt: string): Promise<AiRun> {
  ...
  const data = await response.json();
  const text = (data?.candidates ?? []).flatMap((candidate: any) => candidate?.content?.parts ?? []).map((part: any) => part?.text).filter((value: unknown): value is string => typeof value === 'string').join('\n').trim();
  if (!text) throw new Error('Gemini returned no text output.');
  const sources = (data?.candidates ?? []).flatMap((candidate: any) => candidate?.groundingMetadata?.groundingChunks ?? []).map((chunk: any) => chunk?.web?.uri).filter((url: unknown): url is string => typeof url === 'string') as string[];
...
async function runAnthropic(prompt: string): Promise<AiRun> {
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model, max_tokens: 12000, messages: [{ role: 'user', content: `${jsonInstruction}\n\n${prompt}` }], tools: [{ type: 'web_search_20260318', name: 'web_search', max_uses: 8 }] }),
  });
  if (!response.ok) throw new Error(`Anthropic API ${response.status}: ${await response.text()}`);
  const data = await response.json();
  const text = (data?.content ?? []).map((item: any) => item?.text).filter((value: unknown): value is string => typeof value === 'string').join('\n').trim();
  if (!text) throw new Error('Anthropic returned no text output.');
  const sources = (data?.content ?? []).filter((item: any) => item?.type === 'web_search_tool_result').flatMap((item: any) => item?.content ?? []).map((source: any) => source?.url).filter((url: unknown): url is string => typeof url === 'string') as string[];
