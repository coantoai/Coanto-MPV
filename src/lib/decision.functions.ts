...
function arr(value: unknown): Record<string, unknown>[] {
  return Array.isArray(value) ? value.filter((x): x is Record<string, unknown> => Boolean(x) && typeof x === "object" && !Array.isArray(x)) : [];
}
...
function confidenceOf(result: Record<string, unknown>): Decision["confidence"] {
  ...
  const strength = text(metadata.evidenceStrength || obj(result.snapshot).evidenceStrength).toLowerCase();
...
export function buildDecision(row: Row): Decision {
  ...
  const metadata = obj(result.metadata);
  const evidenceCount = Math.max(0, num(metadata.evidenceCount || metadata.sourceCount));
  const basis = [
    ...arr(result.trust).map(x => text(x.detail || x.title)).filter(Boolean),
    ...arr(result.signals).slice(0, 3).map(x => text(x.evidence || x.detail || x.title)).filter(Boolean),
  ].slice(0, 5);
  return { id: row.id, storeUrl: row.store_url, createdAt: row.created_at, title, action, rationale, priority, confidence: confidenceOf(result), evidenceCount, evidenceBasis: basis };
}

export const getLatestDecision = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<Decision | null> => {
    const { data, error } = await context.supabase.from("analyses").select("id, store_url, created_at, result_json").eq("user_id", context.userId).order("created_at", { ascending: false }).limit(1).maybeSingle();
