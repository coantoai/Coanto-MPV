...
function json(request: Request, traceId: string, body: unknown, status = 200, extraHeaders: HeadersInit = {}) {
  return new Response(JSON.stringify(body), { status, headers: apiSecurityHeaders(request, { 'content-type': 'application/json; charset=utf-8', 'x-request-id': traceId, ...extraHeaders }) });
}
function cachedAnalysis(result: Record<string, unknown>, traceId: string, runId: string, cacheExpiresAt: string) {
  const cloned = structuredClone(result); const metadata = cloned['metadata'] && typeof cloned['metadata'] === 'object' && !Array.isArray(cloned['metadata']) ? { ...(cloned['metadata'] as Record<string, unknown>) } : {};
  Object.assign(metadata, { requestId: traceId, cacheHit: true, cachedRunId: runId, cacheExpiresAt }); cloned['metadata'] = metadata; return cloned;
}
...
export const Route = createFileRoute('/api/analyze')({ server: { handlers: { POST: async ({ request }) => {
  ...
  try {
    ...
    const urlValidation = validateTargetUrl(rawStoreUrl); if (!urlValidation.ok || !urlValidation.url) return json(request, traceId, { error: urlValidation.reason }, 400); const normalizedStoreUrl = normalizeUrl(urlValidation.url);
    ...
    let main; try { main = await getMainSnapshot(normalizedStoreUrl); } catch { await failRun('EVIDENCE_COLLECTION_FAILED'); return json(request, traceId, { error: 'تعذّر جمع أدلة عامة عن الموقع.' }, 422); }
    let discovered = await discoverCompetitors(main, competitors); let commercialCompetitors = filterCommercialCompetitors(main, discovered, competitors); let discoveryMethod = 'search-provider';
    if (!commercialCompetitors.length && process.env['GEMINI_API_KEY']?.trim()) {
      const grounded = await discoverCompetitorsWithGemini(main); discovered = [...discovered, ...grounded]; commercialCompetitors = filterCommercialCompetitors(main, discovered, competitors); discoveryMethod = grounded.length ? 'gemini-google-search' : 'search-provider';
    }
    if (!commercialCompetitors.length) { await failRun('NO_VERIFIED_COMPETITORS'); return json(request, traceId, { error: 'لم نتمكن من التحقق من منافسين مناسبين لهذا الموقع حتى الآن. لم يتم اختراع نتائج.' }, 422); }
    ...
    let ai; try { ai = await runResearchAnalysis(prompt); } catch (error) { console.error('AI analysis failed', { traceId, error }); await failRun('AI_PROVIDER_FAILED'); return json(request, traceId, { error: 'تعذّر تشغيل محرك التحليل حاليًا.' }, 502); }
    let parsedAi: Record<string, unknown>; try { parsedAi = validateAiOutput(parseJsonBlock(ai.text)); } catch (error) { console.error('AI output contract failed', { traceId, error }); await failRun('AI_CONTRACT_FAILED'); return json(request, traceId, { error: 'محرك التحليل أعاد نتيجة غير مكتملة. لم يتم عرض نتيجة غير موثوقة.' }, 502); }
    const analysis = enforceEvidence(parsedAi, main, commercialCompetitors, ai.sources); const sources = [main, ...commercialCompetitors]; const directCount = sources.filter((source) => source.sourceType === 'direct-site').length; const indexedCount = sources.filter((source) => source.sourceType === 'search-index').length;
  ...
  } catch (error) { await failRun('UNEXPECTED_ANALYSIS_FAILURE'); console.error('Analysis request failed', { traceId, error }); return json(request, traceId, { error: 'حدث خطأ أثناء التحليل. حاول مرة أخرى.' }, 500); }
} } } });
