...
import { discoverCompetitors, buildPrompt, getMainSnapshot, hostname, normalizeUrl, validateTargetUrl, type AnalyzeInput, type SiteSnapshot } from '@/lib/analyze.server';
...
function discoveryError(status: string, candidates: number, snapshots: number, accepted: number, totalDiscovered: number) {
  if (accepted > 0) return { code: 'DISCOVERY_OK', stage: 'complete' };
  if (totalDiscovered > 0) return { code: 'DISCOVERY_FILTER_REJECTED', stage: 'competitor-filter' };
  if (candidates > 0 && snapshots === 0) return { code: 'DISCOVERY_URL_VERIFY_FAILED', stage: 'competitor-sites' };
  if (status === 'missing-key') return { code: 'DISCOVERY_NO_KEY', stage: 'gemini-key' };
  if (status === 'provider-error') return { code: 'DISCOVERY_PROVIDER_ERROR', stage: 'gemini-request' };
  if (status === 'empty' || candidates === 0) return { code: 'DISCOVERY_EMPTY', stage: 'gemini-output' };
  return { code: 'NO_VERIFIED_COMPETITORS', stage: 'unknown' };
}

function discoveryMessage(code: string) {
  if (code === 'DISCOVERY_NO_KEY') return 'مفتاح Gemini غير متاح لمحرك الاكتشاف.';
  if (code === 'DISCOVERY_PROVIDER_ERROR') return 'تعذّر اتصال COANTO بمحرك Gemini / Google Search.';
  if (code === 'DISCOVERY_EMPTY') return 'تم تشغيل البحث، لكن لم يرجع منافسين موثوقين بما يكفي.';
  if (code === 'DISCOVERY_URL_VERIFY_FAILED') return 'وجدنا مرشحين، لكن تعذّر تثبيت أدلة عامة كافية على مواقعهم.';
  if (code === 'DISCOVERY_FILTER_REJECTED') return 'وجدنا مرشحين، لكن بوابة التحقق رفضتهم لأن الصلة أو الدليل غير كافيين.';
  return 'لم نتمكن من التحقق من منافسين مناسبين لهذا الموقع حتى الآن. لم يتم اختراع نتائج.';
}

function diagnosticMessage(message: string, code: string) {
  return `${message} — رمز التشخيص: ${code}`;
}
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const failRun = async (code: string) => {
          if (!activeRun) return;
          try {
            await failAnalysisOperation(activeRun.userId, activeRun.runId, code);
          } catch (error) {
            console.error('Analysis failure ledger write failed', { traceId, error });
          }
          activeRun = null;
        };
        ...
        try {
          ...
          if (utf8TooLarge(rawBody, MAX_REQUEST_BYTES)) return json(request, traceId, { error: 'حجم الطلب كبير جدًا.' }, 413);

          let body: Partial<AnalyzeInput>;
          try {
            body = JSON.parse(rawBody) as Partial<AnalyzeInput>;
          } catch {
            return json(request, traceId, { error: 'بيانات الطلب غير صالحة.' }, 400);
          }
          ...
          const inputHash = analysisInputHash({ storeUrl: normalizedStoreUrl, competitors, businessContextUpdatedAt: businessContext.updatedAt });

          let reservation;
          try {
            reservation = await reserveAnalysisOperation({ userId, inputHash });
          } catch (error) {
          ...
          try {
            main = await getMainSnapshot(normalizedStoreUrl);
          } catch {
            await failRun('EVIDENCE_COLLECTION_FAILED');
            return json(request, traceId, { error: diagnosticMessage('تعذّر جمع أدلة عامة عن الموقع.', 'EVIDENCE_COLLECTION_FAILED'), code: 'EVIDENCE_COLLECTION_FAILED', stage: 'target-site' }, 422);
          }

          let discovered: SiteSnapshot[] = [];
          let commercialCompetitors: SiteSnapshot[] = [];
          let discoveryMethod = 'gemini-google-search';
          const grounded = await discoverCompetitorsWithGemini(main);
          const geminiSnapshots = grounded.snapshots.length;
          const geminiDiscovery = { candidateCount: grounded.candidateCount, searchQueries: grounded.searchQueries, status: grounded.status };
          discovered = mergeSnapshots(discovered, grounded.snapshots);
          commercialCompetitors = filterCommercialCompetitors(main, discovered, competitors);
          ...
          if (!commercialCompetitors.length) {
            const legacy = await discoverCompetitors(main, competitors);
            ...
            console.info('Legacy competitor fallback', { traceId, legacyCandidates: legacy.length, totalCandidates: discovered.length, accepted: commercialCompetitors.length });
          }

          if (!commercialCompetitors.length) {
            const diagnostic = discoveryError(geminiDiscovery.status, geminiDiscovery.candidateCount, geminiSnapshots, commercialCompetitors.length, discovered.length);
            await failRun(diagnostic.code);
            return json(request, traceId, {
              error: diagnosticMessage(discoveryMessage(diagnostic.code), diagnostic.code),
              ...diagnostic,
              diagnostic: { geminiStatus: geminiDiscovery.status, candidates: geminiDiscovery.candidateCount, verifiedSites: geminiSnapshots, totalDiscovered: discovered.length },
            }, 422);
          }
          ...
          let ai;
          try {
            ai = await runResearchAnalysis(prompt);
          } catch (error) {
            console.error('AI analysis failed', { traceId, error });
            await failRun('AI_PROVIDER_FAILED');
            return json(request, traceId, { error: diagnosticMessage('تعذّر تشغيل محرك التحليل حاليًا.', 'AI_PROVIDER_FAILED'), code: 'AI_PROVIDER_FAILED', stage: 'analysis-ai' }, 502);
          }

          let parsedAi: Record<string, unknown>;
          try {
            parsedAi = validateAiOutput(parseJsonObject(ai.text));
          } catch (error) {
            console.error('AI output contract failed', { traceId, error });
            await failRun('AI_CONTRACT_FAILED');
            return json(request, traceId, { error: diagnosticMessage('محرك التحليل أعاد نتيجة غير مكتملة. لم يتم عرض نتيجة غير موثوقة.', 'AI_CONTRACT_FAILED'), code: 'AI_CONTRACT_FAILED', stage: 'analysis-output' }, 502);
          }

          const analysis = enforceEvidence(parsedAi, main, commercialCompetitors, ai.sources);
          const finalCompetitors = Array.isArray(analysis['competitors']) ? analysis['competitors'] : [];
          if (!finalCompetitors.length) {
            await failRun('EVIDENCE_GATE_EMPTY');
            return json(request, traceId, { error: diagnosticMessage('تم تشغيل التحليل، لكن بوابة الأدلة لم تسمح بعرض أي منافس غير موثّق.', 'EVIDENCE_GATE_EMPTY'), code: 'EVIDENCE_GATE_EMPTY', stage: 'evidence-gate' }, 502);
          }

          const sources = [main, ...commercialCompetitors];
          const directCount = sources.filter((site) => site.sourceType === 'direct-site').length;
          const indexedCount = sources.filter((site) => site.sourceType === 'search-index').length;
          ...
          Object.assign(metadata, {
            ...
            freshness: 'الآن',
            caveat: indexedCount
              ? 'بعض الأدلة جاءت من فهارس بحث عامة أو بحث Google الموثق لأن بعض المواقع تمنع الوصول الآلي. لم يتم تجاوز أي حماية؛ نوع الدليل ظاهر في النتيجة.'
              : 'المصادر المتاحة تمت قراءتها مباشرة.',
          });
          analysis['metadata'] = metadata;

          try {
            const saved = await saveAnalysis({ userId, storeUrl: normalizedStoreUrl, result: analysis });
            metadata['id'] = saved.id;
            try {
              const ledger = buildAnalysisEvidenceLedger(main, commercialCompetitors, observedAt);
              ...
              console.error('Evidence ledger persistence failed', { traceId, error });
          ...
          return json(request, traceId, { error: diagnosticMessage('حدث خطأ أثناء التحليل. حاول مرة أخرى.', 'UNEXPECTED_ANALYSIS_FAILURE'), code: 'UNEXPECTED_ANALYSIS_FAILURE' }, 500);
        }
      },
    },
  },
