...
export const Route = createFileRoute("/api/analyze")({server:{handlers:{POST:async({request})=>{try{
  ...
  const storeUrl=typeof body.storeUrl==="string"?body.storeUrl.trim():"";const competitors=Array.isArray(body.competitors)?body.competitors.filter((item):item is string=>typeof item==="string").slice(0,10):[];if(!storeUrl)return json({error:"storeUrl مطلوب"},400);
  ...
  let ai;try{ai=await runResearchAnalysis(buildPrompt(main,commercialCompetitors));}catch(error){console.error("AI analysis failed",error);return json({error:"تعذّر تشغيل محرك التحليل حاليًا."},502);}
